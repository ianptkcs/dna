...
test_help_exits_zero_and_prints_usage() {
  local out rc
  out="$("$DNA_ROOT/install/install.sh" --help 2>&1)"; rc=$?
...
test_list_exits_zero_and_lists_all_steps() {
  ...
  statedir="$(mktemp -d)"
  out="$(STATE_DIR="$statedir" "$DNA_ROOT/install/install.sh" --list 2>&1)"; rc=$?
  rm -rf "$statedir"
...
test_invalid_gpu_mode_rejected() {
  local rc
  "$DNA_ROOT/install/install.sh" --gpu-mode=bogus --list >/dev/null 2>&1
  rc=$?
  assert_false [ "$rc" = 0 ]
}
...
test_unknown_flag_rejected() {
  local rc
  "$DNA_ROOT/install/install.sh" --isso-nao-existe >/dev/null 2>&1
  rc=$?
  assert_false [ "$rc" = 0 ]
}
...
test_list_marks_done_steps() {
  ...
  out="$(STATE_DIR="$statedir" "$DNA_ROOT/install/install.sh" --list 2>&1)"
  rm -rf "$statedir"
